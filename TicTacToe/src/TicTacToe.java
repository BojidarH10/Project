...
public class TicTacToe implements ActionListener{
	...
	TicTacToe(){
		...
		textfield.add(resButton);
		
		title_panel.setLayout(new BorderLayout());
		title_panel.setBounds(0,0,650,100);

		resButton.setBounds(650, 20, 100, 50);
		resButton.setText("Restart");
		resButton.addActionListener(this);
		resButton.setFocusable(false);
		resButton.setBackground(new Color(25,255,255));
		resButton.setFont(new Font("Times New Roman",Font.BOLD,20));
		...
		for(int i=0;i<9;i++) {
			...
			buttons[i].setFont(new Font("Cursive",Font.BOLD,120));
			...
			if(e.getSource()==buttons[i]) {
				if(player1_turn) {
					if(buttons[i].getText()=="") {
						buttons[i].setForeground(new Color(111,0,0));
						buttons[i].setText("X");
						player1_turn=false;
					...
					if(buttons[i].getText()=="") {
						buttons[i].setForeground(new Color(0,0,111));
						buttons[i].setText("O");
						player1_turn=true;
